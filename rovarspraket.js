...
function isVowel(char) {
    ...
    if (typeof char !== 'string') {
        ...
        potential = char.toUpperCase();
        for (var index in vowels) {
            if (potential === vowels[index]) {
                return true;
            }
        }
    return false;
...
function rovarspraket(string) {
    ...
    for (var letter in string) {
        rovstring += string[letter];
        if (!isVowel(string[letter])) {
            rovstring += "o";
            rovstring += string[letter];
        }
    }
    // console.log(rovstring)
    return rovstring;
}
...
function reverse(str) {
    var stringCopy = str.split('')
    var reverseCopy = []
    for (var i=0;i<str.length; i++) {
    	reverseCopy.push(stringCopy.pop())
    }
 	return reverseCopy.join('');
}
...
function findLongestWord(sentence) {
    var parsedSentence = sentence.replace("'","")
    var stringArray = parsedSentence.split(" ");
